#!/usr/bin/env python3
"""Emit src/pam50.inc from the EMBOSS EPAM50 matrix.

The values below are the EMBOSS ``EPAM50`` substitution matrix (PAM 50,
scale = ln(2)/2), in the same 24-symbol order as BLOSUM62. We validate the grid
(square, symmetric, documented anchors) before writing, so a transcription slip
fails loudly instead of shipping a wrong matrix. Run once; the .inc is committed.

  python bench/gen_pam50.py
"""
from pathlib import Path

ORDER = "A R N D C Q E G H I L K M F P S T W Y V B Z X *".split()

# EMBOSS EPAM50 (PAM 50). Source: EMBOSS data/EPAM50.
ROWS = {
...
N = len(ORDER)
M = [ROWS[a] for a in ORDER]

# Validate: square, symmetric, and EMBOSS-documented anchors hold.
assert all(len(r) == N for r in M), "non-square matrix"
for i in range(N):
    for j in range(N):
        assert M[i][j] == M[j][i], f"asymmetric at {ORDER[i]},{ORDER[j]}: {M[i][j]} != {M[j][i]}"
flat = [v for r in M for v in r]
assert max(flat) == 13 and min(flat) == -13, f"anchor mismatch: max={max(flat)} min={min(flat)}"
assert M[ORDER.index("W")][ORDER.index("W")] == 13, "W·W should be the +13 high score"

inc = Path("src/pam50.inc")
out = ["// PAM50 substitution scores (similarity, higher = more similar).",
       ...
       f"static const int32_t kPam50Size = {N};",
       f"static const int32_t kPam50[{N} * {N}] = {{",
       "//   " + "   ".join(f"{a:>2}" for a in ORDER)]
for a in ORDER:
    out.append("    " + ", ".join(f"{v:>3}" for v in ROWS[a]) + ",")
out.append("};")
inc.write_text("\n".join(out) + "\n")
print(f"wrote {inc} ({N}x{N}, symmetric, anchors ok)")
