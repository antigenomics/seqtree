...
\begin{remark}[Thymic-selection correction]\label{rem:thymic}
The generation probability $\Pgen$ overcounts relative to the naive-repertoire frequency because
only a fraction of generated receptors survive thymic selection. On average this fraction is
approximately constant, so an effective pre-immune frequency is well approximated by
$f(x) \approx q\,\Pgen(x)$ with a global thymic-selection factor $q \approx 1/2.7$; baseline
antigen-specific T-cell frequencies estimated this way agree with observed
repertoires~\cite{Pogorelyy2018}. Because $q$ is (to first order) sequence-independent, it cancels in
ratios and in the control-calibrated estimator $\hat\pi$ of \S\ref{sec:null}; it matters only when an
\emph{absolute} generation-based frequency is needed (e.g.\ the analytic
$\hat\pi_{\mathrm{gen}}$ fallback for rare queries, \S\ref{sec:precision}), where one uses
$q\,\Pgen$ in place of $\Pgen$.
\end{remark}
