...
\subsection{Reverse problem: peptide $\to$ presenting allele}
...
(\texttt{layout.presentation\_features(register=\textquotesingle anchored\textquotesingle)}): pick the
...
\paragraph{Empirical (\texttt{bench/bench\_mhc\_guess.py}).} On VDJdb/IEDB \texttt{pmhc\_data},
evaluated as a per-(peptide, allele) binary task --- exactly how \texttt{vdjmatch} scores TCR-antigen
specificity --- and split by species, the vote-fraction ranking recovers the restricting allele with
