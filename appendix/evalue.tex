...
the binding motif. For a peptide of unknown restriction, the per-allele anchor-signature value $E_a$
ranks alleles and a global $E_{\mathrm{glob}}=\sum_a E_a$ (Poisson over the allele panel) reports the
expected number of panel alleles presenting it by chance --- a lightweight presentation prior, not a
substitute for a trained predictor.

\paragraph{Empirical (\texttt{bench/bench\_mhc\_guess.py}).} On VDJdb/IEDB \texttt{pmhc\_data}
(alleles with $\ge200$ peptides), widening the scope on the anchor signature until each held-out
peptide has $10$--$100$ non-exact neighbours and voting their alleles recovers the restricting allele
far above chance, and the aggregate E-value cleanly separates real peptides from length-matched
random ``noise'' (Table~\ref{tab:mhcguess}, Fig.~\ref{fig:mhcguess}). Crucially, this works only with
\emph{presentation} (anchor) features: the TCR-facing homology used for cross-reactivity carries no
allele information (AUROC $\approx0.5$), confirming that MHC restriction lives in the anchors, not the
TCR-facing surface.
...
MHC-II & 15  & 0.34 & 0.067 & 0.58 & 1.2\phantom{0} / 1.5\phantom{0}\\
\hline
\end{tabular}
\caption{Allele guessing from presentation-signature neighbours ($\sim$800 held-out peptides per
class). Top-1 accuracy is $\sim$5--40$\times$ chance ($1/$alleles); the aggregate E-value separates
presented peptides (low $E$) from random peptides (high $E$), so the latter are rejected as noise by
an $E$ threshold.}
\label{tab:mhcguess}
\end{table}

\begin{figure}[ht]\centering
  \includegraphics[width=\linewidth]{mhc_guess.pdf}
  \caption{MHC-allele guessing (\texttt{bench/bench\_mhc\_guess.py}): fraction of peptides called
  confident vs the aggregate E-value threshold, for real presented peptides (red) and length-matched
  random peptides (blue), MHC-I (left) and MHC-II (right). The gap is the noise-rejection margin.}
  \label{fig:mhcguess}
\end{figure}
