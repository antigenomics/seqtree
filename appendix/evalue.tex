...
% Compact author/affiliation block (authblk; affiliations italicised by affil-it).
...
\author[1,2,3]{Mikhail Shugay\thanks{\href{mailto:mikhail.shugay@gmail.com}{mikhail.shugay@gmail.com}}}
\affil[1]{Institute of Translational Medicine, Russian National Medical State University, Moscow, Russia}
\affil[2]{Department of Genomics of Adaptive Immunity, Shemyakin--Ovchinnikov Institute of Bioorganic Chemistry RAS, Moscow, Russia}
\affil[3]{Institute of Molecular Biology NAS RA, Erevan, Armenia}
\date{\small\textit{seqtree technical appendix}}
...
\begin{figure}[t]
  ...
  \caption{Epitope detection complexity (\texttt{bench/bench\_epitope.py}). Within-set neighbour
  structure (top) and the subsampled fraction called significant (Benjamini--Hochberg, FDR $<0.05$)
  versus sampled depth $n$ (bottom), for the convergent GIL and the diffuse NLV A*02 repertoires at
  scope $\theta=1$. GIL's dominant cluster is recovered an order of magnitude sooner than NLV's
  diffuse one, matching Eq.~\eqref{eq:phi}.}
  \label{fig:epitope}
\end{figure}
